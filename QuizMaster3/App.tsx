      ...
      case 'idle':
        return (
          <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in duration-500 flex flex-col items-center">
            ...
            {/* Восстановленный красивый блок подсказки в столбец */}
            <div className="max-w-md w-full bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-3xl p-8 shadow-sm mb-16">
              ...
              <p className="text-slate-800 dark:text-amber-200 mb-6 text-sm leading-relaxed">
                Иногда в документах Word используются «мягкие переносы», которые мешают распознать структуру. Исправьте это в Word за 10 секунд:
              </p>
              <div className="bg-white/60 dark:bg-slate-900/60 p-6 rounded-2xl border border-amber-200/50 dark:border-amber-900/30 space-y-5">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <p className="text-sm"><b>Найти:</b> <code className="bg-amber-100 dark:bg-amber-900/50 px-1.5 py-0.5 font-bold rounded text-amber-900 dark:text-amber-400">^l</code> (строчная L)</p>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <p className="text-sm"><b>Заменить на:</b> <code className="bg-amber-100 dark:bg-amber-900/50 px-1.5 py-0.5 font-bold rounded text-amber-900 dark:text-amber-400">^p</code></p>
                </div>
                <p className="pt-2 text-[11px] text-slate-400 italic text-center">Затем нажмите "Заменить все" и сохраните файл.</p>
              </div>
            </div>
          ...
          <div className="max-w-5xl mx-auto py-12 px-4 animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               <button onClick={() => startSession('test')} className="group p-8 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[35px] text-left hover:border-indigo-500 hover:shadow-xl transition-all h-full flex flex-col">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-xs flex-grow">25 случайных вопросов для быстрой проверки.</p>
               </button>
               ...
               <button onClick={() => startSession('preparation')} className="group p-8 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[35px] text-left hover:border-emerald-500 hover:shadow-xl transition-all h-full flex flex-col">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-xs flex-grow">Только неизученные вопросы. Методичное запоминание.</p>
               </button>
               ...
               >
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-transform ${favoritesCount === 0 ? 'bg-slate-200 text-slate-400' : 'bg-amber-50 dark:bg-amber-900/40 text-amber-500 group-hover:scale-110'}`}>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-xs flex-grow">Вопросы, которые вы пометили звездочкой ({favoritesCount}).</p>
               </button>
            </div>
          ...
          <div className="max-w-3xl mx-auto py-8 px-4 animate-in slide-in-from-bottom-8 duration-500">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => toggleBookmark(q.id)} className={`p-2 rounded-xl border transition-all ${isBookmarked ? 'bg-amber-50 border-amber-200 text-amber-500' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-300'}`}>
          ...
          <div className="max-w-2xl mx-auto py-16 px-4 text-center animate-in zoom-in-95 duration-500">
             ...
             <div className="grid grid-cols-2 gap-6 mb-16">
               ...
               <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-xl">
                 ...
                 <p className="text-xs font-bold text-slate-400 uppercase mt-2">Верно</p>
               </div>
             </div>
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-2xl p-5 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="mb-4">
                    ...
                    <p className="text-sm font-bold dark:text-white">{isLoggedIn ? "Пользователь браузера" : "Гость"}</p>
                  </div>
                  {!isLoggedIn ? (
                    ...
                    <Button variant="outline" fullWidth onClick={() => { setIsLoggedIn(false); setShowUserMenu(false); }} className="rounded-xl text-xs py-2.5">Выйти из сеанса</Button>
                  )}
                  <p className="mt-3 text-[10px] text-slate-400 text-center leading-tight">Данные сохраняются локально на этом устройстве.</p>
                </div>
              )}
            </div>
          </div>
